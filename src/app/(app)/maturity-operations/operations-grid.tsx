              ...
              {visible.map((row, rowIndex) => {
                ...
                return (
                  <tr key={row.id} className={cn('group hover:bg-[color-mix(in_oklab,var(--color-brand-500)_7%,var(--surface-solid))]', rowSurface)}>
                    ...
                    {show('due') && <td className={cell}><EditableCell row={row.id} col="due" type="money" value={rupees(row.duePaise)} disabled={!canSchedule && !canEdit} onCommit={(v) => savePlanned(row, v)} /></td>}
                    {show('recommended') && <td className={cell}><EditableCell row={row.id} col="recommended" type="money" value={rupees(row.recommendedPaise)} disabled onCommit={async () => {}} /></td>}
